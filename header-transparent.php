<?php
/**
 * The template for displaying the transparent header - this is for demo purposes only as the header type is actually controlled by a php switch block based on customizer choice, with classes doing all the styling. In here, we remove that switch block and simplify the header as much as we can
 *
...
<html class="no-js" <?php language_attributes();?> >
	...
	<body <?php
if ((is_page('sidebar-on-the-right')) || (is_page('sidebar-on-the-left')) || (is_page('sticky-sidebar-2')) || is_page('video-background')) {body_class('transparent-header page-template-demo-header-transparent');} else if (is_page('sidebar-off')) {body_class('transparent-header page-template-demo-header-transparent has-sidebar-off');} else if (is_page('post-layout-1')) {body_class('transparent-header no-full-width-entry-title featured-hero-title-below page-template-demo-header-transparent');} else if (is_page('post-layout-2')) {body_class('transparent-header has-full-width-entry-title featured-hero-title-below page-template-demo-header-transparent');} else if (is_page('post-layout-7')) {body_class('transparent-header has-full-width-entry-title featured-hero-title-contained page-template-demo-header-transparent');} else {body_class('transparent-header');}
?>>
...
<!-- This is so that we can evenly space items in the flex parent ( the dummy item is given the same width as it's opposite counterpart, and hidden from view) -->
	...
	<header class="transparent-header site-header" role="banner">
